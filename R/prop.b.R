propClass <- if (requireNamespace('jmvcore', quietly=TRUE)) R6::R6Class(
    ...
    private = list(
      ...
      .run = function() {
        ...
        Sup_an <- Sup - Sup_n
        Supc_an <- Sup_an - Ac(self$options$correction,1,len)
        ...
        if (isTRUE(self$options$bi) & isTRUE(len==2)) {
          ...
          suppressWarnings(xmin1x <- optimize(f, c(0, p), tol = toler, a, r, p, goalx))
          suppressWarnings(xmin2x <- optimize(f, c(p, 1), tol = toler, a, r, p, goalx))
          xmin <- xmin1x$minimum
          xmax <- xmin2x$minimum
          ...
          g <- data.frame(p=p, a=a, r=r, expprop=expProps[1],
                          xmin1L=xmin1L$minimum, xmin2L=xmin2L$minimum, xmin=xmin, xmax=xmax, goal=goal)
      ...
      .plotc=function(imagec, ...) {
        ...
        if(self$options$plotype=="lplot") {
          ...
          plot <- curve(g$a*log(x)+g$r*log(1-x)-(g$a*log(g$p)+g$r*log(1-g$p)), xlim = c(g$xmin,g$xmax), 
