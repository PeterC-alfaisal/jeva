...
llrOptions <- if (requireNamespace("jmvcore", quietly=TRUE)) R6::R6Class(
    "llrOptions",
...
llrResults <- if (requireNamespace("jmvcore", quietly=TRUE)) R6::R6Class(
    "llrResults",
    ...
    public=list(
        initialize=function(options) {
            ...
            self$add(jmvcore::Image$new(
                options=options,
                name="plotc",
                title="`Likelihood curve for RR with S-{lint} support interval`",
                width=500,
                height=400,
                renderFun=".plotc",
                clearWith=list(
    ...
    public = list(
        initialize = function(options, data=NULL, datasetId="", analysisId="", revision=0) {
            super$initialize(
                ...
                version = c(1,0,0),
                options = options,
                results = llrResults$new(options=options),
                data = data,
                datasetId = datasetId,
...
llr <- function(
    ...
    if ( ! requireNamespace("jmvcore", quietly=TRUE))
        stop("llr requires jmvcore to be installed (restart may be required)")
