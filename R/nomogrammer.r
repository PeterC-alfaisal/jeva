...
##   December 2016
##   Subsequently from meddecide jamovi module of Serdar Balci https://github.com/sbalci/meddecide/blob/master/R/nomogrammer.r
...
p <- ggplot(df) +
        ...
        annotate(geom = "text",
                 ...
                 label = ticks_lrs,
                 size = rel(LabelSize)) +
        annotate(geom = "text", label = "Likelihood Ratio") +
        annotate(geom="point",
...
detailedAnnotation <- paste(
    paste0("Prior probability (Prevalence) = ", p2percent(prior_prob)),
    paste0("LR = ", signif(PLR, 3),",   Neg LR = ", signif(NLR, 3)),
    paste("Posterior Positive PPV =", p2percent(post_prob_pos)),
    paste("False Negative 1-NPV =", p2percent(post_prob_neg)),
    sep = "\n")
