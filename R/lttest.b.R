...
lttestClass <- if (requireNamespace('jmvcore', quietly=TRUE)) R6::R6Class(
    ...
    private = list(
      .init = function() {      
        ...
        table$setRow(rowNo=1, values=list(var= "<i>H</i>\u2080 vs observed mean"))
        table$setRow(rowNo=2, values=list(var="<i>H</i>\u2090 vs observed mean"))
        ...
        .run = function() {
          ...
          table$setRow(rowNo=3, values=list(Value="", 
                      mdiff= self$options$alt-self$options$nul, SE=sed, S=S_1-S_m, t="", df="", p=""))
      ...
      .populateSupportText = function(st) {
        ...
        Sxl = list(s=st$S1, "<i>H</i>\u2080", "observed mean")                       
