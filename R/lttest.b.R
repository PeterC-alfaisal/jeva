...
lttestClass <- if (requireNamespace('jmvcore', quietly=TRUE)) R6::R6Class(
    ...
    private = list(
      .init = function() {      
        ...
        table$setRow(rowNo=1, values=list(var= "H\u2080 vs observed mean"))
        table$setRow(rowNo=2, values=list(var="H\u2090 vs observed mean"))
      ...
      .run = function() {
        ...
        table$setRow(rowNo=1, values=list(Value=results$null.value, 
                                          mdiff= self$options$nul-m.obs, SE=sed, S=S_m-correct, 
                                          Param=paste0(c(k1,k2), collapse = ', '), t=tval, df=df, p=results$p.value))
        table$setRow(rowNo=2, values=list(Value=results1$null.value, 
                                          mdiff= self$options$alt-m.obs, SE=sed, S=S_1, 
                                          Param=paste0(c(k2,k2), collapse = ', '), t=results1$statistic, df=df, p=results1$p.value))
        table$setRow(rowNo=3, values=list(Value="", 
                                          mdiff= self$options$alt-self$options$nul, SE=sed, S=S_1-S_m+correct, 
                                          Param=paste0(c(k2,k1), collapse = ', '), t="", df="", p=""))
      ...
      .plot=function(image, ggtheme, theme, ...) {
        ...
        plot <- ggplot(plotData, aes(x="", y=mean)) +
          geom_errorbar(aes(ymin=lolim, ymax=hilim, width=.1), colour=theme$color[2]) +
      ...
      .populateSupportText = function(st) {
        ...
        Sxl = list(s=st$S1-st$correct, "<i>H</i>\u2080", "observed mean")                       
        stg1 <- private$.strength(Sxl)
        Sxl = list(s=st$S2, "<i>H</i>\u2090", "observed mean")                       
        ...
        str = paste0("<br> <h2>Summarizing the evidential analysis</h2>", "<br>",
                     ...
                     "<p>Observed mean = ", signif(st$mobs,3), ", with an <i>S</i>-",st$lint, " 
                     support interval was from ", signif(st$lolim,3), " to ", signif(st$hilim,3), 
                     ". 
                     <p> Also give the sample size and a graphical plot that includes support 
