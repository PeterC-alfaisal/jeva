l2sttestClass <- if (requireNamespace('jmvcore', quietly=TRUE)) R6::R6Class(
    ...
    private = list(
      .init = function() {   
        ...
        if(isTRUE (self$options$welch)) notext <- paste(notext," Uses Welch's procedure for unequal variances")
      ...
      .run = function() {
        ...
        table$setRow(rowNo=3, values=list(Value="", mdiff= self$options$alt-self$options$nul, sed=sed, 
                                          S=S_1-S_m+correct, Param=paste0(c(k2,k1), collapse = ', '),
                                          t="", df="", p=""))
        # stats for summary        
        stats <- list(S1 = S_m,
                      S2 = S_1,
      ...
      .populateSupportText = function(st) {
        ...
        str = paste0("<br> <h2>Summarizing the evidential analysis</h2>", "<br>",
                     ...
                     <br><br>
                     There are no thresholds for <i>S</i> values, just guidelines on the strength 
                     of evidence for one hypothesis versus the other. They range from 
